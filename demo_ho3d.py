...
def training(config):
    ...
    debug_dir = '/mnt/sda1/lxy/debug_render/' + 'ho3d_final_demo_noise_' + config.tag
    ...
    load_ckpt = os.path.join('/mnt/sda1/lxy/HOGS_results/ho3d-hogs_18seen_demo_gpu0/', "ckpt200000.pth")
    print(load_ckpt)
    scene.load_checkpoint(load_ckpt)
    testing_iterations.append(1)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            validation(iteration, testing_iterations, testing_interval, iteration < model.gaussian.delay, scene, evaluator, debug_dir,
                       (pipe, background))
...
def validation(iteration, testing_iterations, testing_interval, delay, scene: Scene, evaluator, debug_dir, renderArgs):
    ...
    for config in validation_configs:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                render_pkg = render(data, iteration, scene, *renderArgs, compute_loss=True, return_opacity=True, delay=delay,render_bg=True)
                examples = []
                image = torch.clamp(render_pkg["render"], 0.0, 1.0)
                gt_image = torch.clamp(data.original_image.to("cuda"), 0.0, 1.0)
                opacity_image = torch.clamp(render_pkg["opacity_render"], 0.0, 1.0)
                ...
                full_gt_image = torch.clamp(data.full_image.to("cuda"), 0.0, 1.0)
                full_opacity_image = torch.clamp(render_pkg["full_opacity_render"], 0.0, 1.0)
                bg_render = torch.clamp(render_pkg["bg_render"], 0.0, 1.0)

                if iteration==1 or iteration>=200000:
                    # full_render = cv2.cvtColor(
                    #     (full_image.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
                    # full_ground_truth = cv2.cvtColor(
                    #     (full_gt_image.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8),
                    #     cv2.COLOR_BGR2RGB)
                    ...
                    #
                    # cv2.imwrite(
                    #     os.path.join(debug_dir, '{}_full_render_{}.png'.format(idx, str(data.image_name))),
                    #     full_render)
                    # cv2.imwrite(
                    #     os.path.join(debug_dir, '{}_full_ground_truth_{}.png'.format(idx, str(data.image_name))),
                    #     full_ground_truth)
                    # cv2.imwrite(
                    ...
                    full_render = cv2.cvtColor(
                        (full_image.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
                    bg_render = cv2.cvtColor(
                        (bg_render.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
                    full_ground_truth = cv2.cvtColor(
                        (full_gt_image.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8),
                        cv2.COLOR_BGR2RGB)
                    ori_image=cv2.cvtColor(
                        (torch.clamp(data.ori_img.to("cuda"), 0.0, 1.0).permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8),
                        cv2.COLOR_BGR2RGB)
                    ...
                    cv2.imwrite(
                        ...
                        full_render)
                    cv2.imwrite(
                        os.path.join(debug_dir, '{}_bg_render_{}.png'.format(idx, str(data.image_name))),
                        bg_render)
                    cv2.imwrite(
                        os.path.join(debug_dir, '{}_full_ground_truth_{}.png'.format(idx, str(data.image_name))),
                        full_ground_truth)
                    cv2.imwrite(
                        os.path.join(debug_dir, '{}_ori_image_{}.png'.format(idx, str(data.image_name))),
                        ori_image)
...
def main(config):
    ...
    if config.save_code:
        shutil.copyfile('train_ho3d.py', config.exp_dir + '/code/train_ho3d.py')
