...
def extract_mesh(config, ckpt_path, filter_mesh: bool, texture_mesh: bool,
    ...
    with torch.no_grad():
        ...
        load_ckpt = os.path.join(ckpt_path, "ckpt10.pth")
        ...
        first_iter = 0
        # save mesh 20201022-subject-10_15-20201022_113756_932122061900_color_000066
        # save mesh 20201022-subject-10_15-20201022_113756_932122061900_color_000066
        # trainLoader = DataLoader(scene.train_dataset, batch_size=1, shuffle=True, num_workers=8)
        # progress_bar = tqdm(range(trainLoader.__len__()), desc="Evaling progress")
        ...
        #
        #     file_name = str(sub_id) + '_' + str(data['cam_id'][0]) + '_' + str(int(data['frame_id']))
        #
        #
        #     pc_hand, pc_obj, loss_reg, colors_precomp, obj_colors_precomp, updated_camera, canonical_hand, canonical_obj = \
        #         scene.convert_gaussians(data, 360001, compute_loss=True, pose_refine=True, delay=False)
        #     pc_hand._xyz -= updated_camera.hand_root.detach()
        #     pc_obj._xyz -= updated_camera.obj_trans.detach()
        #
        #     render_pkg = render(data, 360001, scene, pipeline, background, compute_loss=True, return_opacity=True,
        #                         pose_refine=True)
        ...
        #
        #     marching_tetrahedra_with_binary_search('/home/cyc/pycharm/lxy/3DGS/debug/', file_name + "_obj.ply", cams,
        #                                            pc_obj, pipeline,
        #                                            background, kernel_size, 0.25, filter_mesh, texture_mesh,
        #                                            near, far,
        #                                            trans=updated_camera.obj_trans.detach().cpu().numpy())
        #
        #     cams = QueryCamerasLoader(scene.metadata['aabb'], cam_num=32).get_cam
        #     # cams = QueryCamerasLoader(scene.metadata['aabb'], trans=torch.tensor(.0),cam_num=12).get_cam
        #     marching_tetrahedra_with_binary_search('/home/cyc/pycharm/lxy/3DGS/debug/', file_name + "_hand.ply", cams, pc_hand, pipeline,
        #                                            background, kernel_size, 0.25, filter_mesh, texture_mesh, near,
        #                                            far, trans=updated_camera.hand_root.detach().cpu().numpy())
        # exit()

        testLoader_6k = DataLoader(scene.test_dataset_6k, batch_size=1, shuffle=False,num_workers=0)
        progress_bar = tqdm(range(testLoader_6k.__len__()), desc="Training progress")
        ...
        for ther in thers:
            ...
            for subject in dataset._SUBJECTS:
                rendered_hand[int(subject.split('-')[-1])] = False
            for idx, data in enumerate(testLoader_6k):
                data = {key: value.to('cuda') if isinstance(value, torch.Tensor) else value
                        for key, value in data.items()}
...
def main(config):
    ...
    parser.add_argument("--ckpt_path", default='/mnt/sda1/lxy/HOGS_results/dexycb-hogs_prerigid_centeredbbox/obj_pose_refine_tanh_1020/')
