...
class SMPLXLayer(SMPLX):
    ...
    def forward(
        ...
        **kwargs
        ...
        '''
        device, dtype = self.shapedirs.device, self.shapedirs.dtypebody
...
class MANO(SMPL):
    ...
    def forward(
        ...
        **kwargs
        ...
        v, joints, joints_a_pose, bone_transforms, abs_bone_transforms, v_posed, v_shaped,rot_mats = lbs(betas, full_pose, self.v_template,
                                                                                            self.shapedirs, self.posedirs,
                                                                                            self.J_regressor, self.parents,
                                                                                            self.lbs_weights, pose2rot=True,
                                                                                            )

        Jtr = joints + transl.unsqueeze(dim=1)
        vertices = v + transl.unsqueeze(dim=1)
        v_posed = v_posed + transl.unsqueeze(dim=1)

        # Add pre-selected extra joints that might be needed
        joints = self.vertex_joint_selector(v, joints)
