...
def check_gpu1_cuda():
    ...
    # 将模型和数据移动到 GPU 1
    model = CLIPModel.from_pretrained("/home/cyc/pycharm/lxy/3DGS/lib/clip-vit-base-patch32/").cuda().to(device)
    input_data = torch.randn(8, 3, 224, 224).to(device)  # 输入形状：[batch=1, channels=3, H=32, W=32]


    # 前向传播
    output = model.vision_model(pixel_values=input_data)
    print(output.last_hidden_state)

    # 检查输出是否为全零（预期结果）
    if torch.all(output.last_hidden_state == 0):
        print("测试通过：输出全零，符合预期。")
    else:
        print(f"测试警告：输出非全零。首元素值为：{output.last_hidden_state[0, 0, 0].item()}")

    # 检查是否存在 NaN/Inf
    if torch.isnan(output.last_hidden_state).any():
        print("错误：输出中存在 NaN！")
    if torch.isinf(output.last_hidden_state).any():
        print("错误：输出中存在 Inf！")
...
def training(config):
    ...
    # while True:
    #     input_data = torch.randn(8, 3, 224, 224).cuda()  # 输入形状：[batch=1, channels=3, H=32, W=32]
    #
    #     # 前向传播
    #     output = scene.converter.backbone.vision_model(pixel_values=input_data)
