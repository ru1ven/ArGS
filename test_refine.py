...
def training(config):
    ...
    scene.converter.body_model_pca = MANO(model_path='/home/cyc/pycharm/lxy/gs/3dgs-avatar-release/hand_models/mano/',
    ...
    if load_ckpt is None:
        load_ckpt = os.path.join(config.ckpt_dir, "ckpt" + str(ckpt_epoch) + ".pth")
    #sscene.load_checkpoint(load_ckpt)
    (gaussian_params, objgaussian_params, converter_sd, converter_opt_sd, converter_scd_sd, first_iter) = torch.load(
        load_ckpt)
    for sub_id in scene.gaussians_hand_group:
        scene.gaussians_hand_group[sub_id].restore(gaussian_params[sub_id], config.opt)
    for obj_id in scene.gaussians_obj_group:
        scene.gaussians_obj_group[obj_id].restore(objgaussian_params[obj_id], config.opt)
    scene.converter.load_state_dict(converter_sd)

    first_iter = 360000

    for sub_id in gaussians_hand_group:
        if config.fix_gaussian:
            gaussians_hand_group[sub_id].refine_setup(opt)
        else:
            scene.gaussians_hand_group[sub_id].training_setup(opt)
    for obj_id in gaussians_obj_group:
        if config.fix_gaussian:
            gaussians_obj_group[obj_id].refine_setup(opt)
        else:
            scene.gaussians_obj_group[obj_id].training_setup(opt)


    #scene.converter.pc_refine_setup()
    scene.converter.set_optimizer()
    # print(scene.converter.optimizer)
    # print(scene.converter.scheduler)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # for group in scene.gaussians_hand_group[data.subject_id].optimizer.param_groups:
        #     if group["name"] == 'opacity':
        #         print(group['params'][0])
        #         print(scene.gaussians_hand_group[data.subject_id].optimizer)
        #         stored_state = scene.gaussians_hand_group[data.subject_id].optimizer.state.get(group['params'][0], None)
        #         print(stored_state)
        ...
        with torch.no_grad():
            ...
            # Densification
            if not config.fix_gaussian and iteration < opt.densify_until_iter and iteration > model.gaussian.delay:
...
def validation(iteration, testing_iterations, testing_interval, scene: Scene, evaluator, renderArgs):
    ...
    for config in validation_configs:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            r_error /= len(config['cameras'])
            t_error /= len(config['cameras'])
            print("\n[ITER {}] Evaluating {}: lpips {} PSNR {} MPJPE {} t_error {} r_error {}".format(iteration, config['name'], lpips_test,
                                                                                psnr_test, mpjpe, t_error, r_error))
            wandb.log({
                ...
                config['name'] + '/loss_viewpoint - r_error': r_error,
                config['name'] + '/loss_viewpoint - t_error': t_error,
            })
...
def main(config):
    ...
    if config.save_code:
        shutil.copyfile('./train.py', config.exp_dir + '/code/test_refine.py')
