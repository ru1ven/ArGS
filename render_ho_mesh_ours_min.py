...
def marching_tetrahedra_with_binary_search(model_path, name, views, gaussians, pipeline, background,
    ...
    render_path = os.path.join(model_path, 'mesh_posed_min_16_'+str(ther), 'sdf_mesh')
    ...
    for step in range(n_binary_steps):
        ...
        mesh.vertices += trans
        #print(os.path.join(render_path, name))
...
def extract_mesh(config, ckpt_path, filter_mesh: bool, texture_mesh: bool,
    ...
    with torch.no_grad():
        ...
        scene = Scene(config, gaussians_hand_group, gaussians_obj_group, config.exp_dir, multi_batch=False)
        scene.eval()

        load_ckpt = os.path.join(ckpt_path, "ckpt360000.pth")
        ...
        progress_bar = tqdm(range(scene.test_dataset_6k.__len__()), desc="Training progress")
        ...
        for ther in thers:
            ...
            for subject in dataset._SUBJECTS:
                rendered_hand[int(subject.split('-')[-1])] = False
            for idx, data_idx in enumerate(list(range(len(scene.test_dataset_6k)))):
                data = scene.test_dataset_6k[data_idx]

                obj_id = int(data.obj_id)
                sub_id = int(data.subject_id)

                file_name = str(sub_id)+'_'+ str(data.cam_id)+'_'+ str(int(data.frame_id))
                if file_name != '7_20200928_155840_839512060362_60':
                    continue
                ...
                if rendered_obj[obj_id] is False or rendered_hand[sub_id] is False:
                    ...
                    pc_hand, pc_obj, loss_reg, colors_precomp, obj_colors_precomp, updated_camera, canonical_hand, canonical_obj = \
                        scene.convert_gaussians(data, 360001, compute_loss=True, pose_refine=False, delay=True)
                    ...
                    # #使用 OpenCV 保存图像
                    # cv2.imwrite('/home/cyc/pycharm/lxy/3DGS/debug/mesh/query_render_baseline.png', image)
...
def main(config):
    ...
    parser.add_argument("--ckpt_path", default='/mnt/sda1/lxy/HOGS_results/dexycb-3dgs-avatar_ho_posecond')
