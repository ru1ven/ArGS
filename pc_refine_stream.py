...
def training(config):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        streams = [torch.cuda.Stream() for _ in range(config.accumulation_steps)]
        results = []
        ...
        for i in range(config.accumulation_steps):
            with torch.cuda.stream(streams[i]):
                ...
                _, _, loss_reg, _, _, _ = scene.convert_gaussians(data, iteration, compute_loss=True)
...
def validation(iteration, testing_iterations, testing_interval, scene: Scene, evaluator, renderArgs):
    ...
    for config in validation_configs:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            print("\n[ITER {}] Evaluating {}: lpips {} PSNR {} MPJPE {} r_error {} t_error {} ".format(iteration, config['name'], lpips_test, psnr_test, mpjpe, r_error, t_error))
            wandb.log({
