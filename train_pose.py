...
from utils.general_utils import fix_random, Evaluator, PSEvaluator, cal_pose_error, relative_pose_error, \
    compute_obj_metrics_dexycb, cfg_from_yaml_file
...
def training(config):
    ...
    #
    scene.converter.kpTR = KeypointTR().cuda()
    ...
    testLoader = DataLoader(scene.test_dataset, batch_size=config.test_batch_size, shuffle=True, num_workers=8)
    ...
    for epoch in range(config.refine_epoch):
        for ii, data in enumerate(trainLoader):
            ...
            #
            #
            # loss_reg["loss_joint_3d"] *= opt.joint_weight
            # loss_reg["loss_joint_cls"] *= opt.cls_weight
            # loss_reg["loss_all_joint_3d"] *= opt.joint_weight
            # loss = sum(loss_reg[k] for k in loss_reg)
            # loss.backward()
            ...
            with torch.no_grad():
                ...
                # Log and save
                #if epoch==0 and ii == 10 or ii == trainLoader.__len__()-1:
                if ii == trainLoader.__len__() - 1:
                    validation(iteration, scene, testLoader)
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                if config['name'] == 'test':
                    ...
                    ADDS, MCE = compute_obj_metrics_dexycb(updated_camera['obj_rots_gt'],
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                if config['name'] == 'test':
                    ...
                    mpjpe += cal_pose_error(render_pkg["updated_camera"], body_model)

                    r_e, OCE = relative_pose_error(render_pkg["updated_camera"])
...
def save_gaussian(iteration, scene: Scene, evaluator, renderArgs):
...
def main(config):
    ...
    # print(config.name)
    config.exp_dir = os.path.join('/home/cyc/pycharm/lxy/3DGS/results', config.name, config.refine_tag)
    config.ckpt_dir = os.path.join('/home/cyc/pycharm/lxy/3DGS/results', config.name)
    ...
    if config.save_code:
        shutil.copyfile('./train_pose.py', config.exp_dir + '/code/train_pose.py')
