...
@torch.no_grad()
def evaluage_alpha_gof(points, views, gaussians, pipeline, background, kernel_size, return_color=False):
    final_alpha = torch.ones((points.shape[0]), dtype=torch.float32, device="cuda")
    if return_color:
        final_color = torch.ones((points.shape[0], 3), dtype=torch.float32, device="cuda")
...
def marching_tetrahedra_with_binary_search(model_path, name, views, gaussians, pipeline, background,
    ...
    render_path = os.path.join(model_path, str(ther), 'sdf_mesh')
...
def extract_mesh(config, ckpt_path, filter_mesh: bool, texture_mesh: bool,
    ...
    with torch.no_grad():
        ...
        load_ckpt = os.path.join(ckpt_path, "ckpt50000.pth")
        ...
        #testLoader = DataLoader(scene.test_dataset, batch_size=1, shuffle=False,num_workers=0)
        cameras = list(range(len(scene.test_dataset)))
        progress_bar = tqdm(len(scene.test_dataset), desc="Training progress")

        thers = [0.4]
        for ther in thers:

            for idx, data_idx in enumerate(cameras):
                ...
                file_name = data.image_name.replace("/",'_')

                pc_hand_r, pc_hand_l,pc_obj, loss_reg, colors_precomp_r, colors_precomp_l, obj_colors_precomp, \
                updated_camera= scene.convert_gaussians(data, 50001+idx, compute_loss=True)

                pc_hand_r._xyz -= updated_camera.Jtrs_r_3d[0].detach()
                pc_hand_l._xyz -= updated_camera.Jtrs_l_3d[0].detach()
                pc_obj._xyz -= updated_camera.obj_trans.detach()


                render_pkg = render(data, 50001+idx, scene, pipeline, background, compute_loss=True, return_opacity=True,
                                    delay=False)
                ...
                mesh_r = open3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
                    open3d.geometry.PointCloud(points=open3d.utility.Vector3dVector(pc_hand_r._xyz.detach().cpu().numpy()))
                    , open3d.utility.DoubleVector(radii)
                )
                mesh_l = open3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
                    open3d.geometry.PointCloud(points=open3d.utility.Vector3dVector(pc_hand_l._xyz.detach().cpu().numpy())),
                    open3d.utility.DoubleVector(radii)
                )
                mesh_o = open3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
                    open3d.geometry.PointCloud(points=open3d.utility.Vector3dVector(pc_obj._xyz.detach().cpu().numpy()))
                    , open3d.utility.DoubleVector(radii)
                )


                os.makedirs(os.path.join(ply_path,'hull_mesh'), exist_ok=True)
                mesh_r.export(os.path.join(ply_path,'hull_mesh', file_name+'_r.obj'))
                mesh_l.export(os.path.join(ply_path, 'hull_mesh', file_name + '_l.obj'))
                mesh_o.export(os.path.join(ply_path, 'hull_mesh', file_name + '_o.obj'))

                # gof
                # cams = QueryCamerasLoader(scene.metadata_obj[obj_id]['obj_aabb'], cam_num=16).get_cam
                #
                ...
                #
                # cams = QueryCamerasLoader(scene.metadata['right']['aabb'], cam_num=16).get_cam
                # marching_tetrahedra_with_binary_search(ply_path, file_name+"_hand_r.ply", cams, pc_hand_r, pipeline,
                #                                        background, kernel_size, ther, filter_mesh, texture_mesh, near, far, trans=updated_camera.Jtrs_r_3d[0].detach().cpu().numpy())
                # cams = QueryCamerasLoader(scene.metadata['left']['aabb'], cam_num=16).get_cam
                # marching_tetrahedra_with_binary_search(ply_path, file_name + "_hand_l.ply", cams, pc_hand_l, pipeline,
                #                                        background, kernel_size, ther, filter_mesh, texture_mesh, near,
                #                                        far, trans=updated_camera.Jtrs_l_3d[0].detach().cpu().numpy())
