...
from utils.general_utils import fix_random, Evaluator, PSEvaluator, cal_pose_error, relative_pose_error, \
    prepare_model_template, compute_obj_metrics_ycb
...
#@profile
def training(config, debug_render=False):
    model = config.model
    ...
    print('Train on {} samples'.format(len(scene.train_dataset)))
    scene.train()
    #scene.eval()
    ...
    if debug_render:
        debug(scene,scene.test_dataset_25k, evaluator, config.name,(pipe, background))
        exit()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            validation(iteration, testing_iterations, testing_interval, iteration < model.gaussian.delay,scene, scene.test_dataset_25k, evaluator, (pipe, background))
            ...
            if iteration in checkpoint_iterations:
                # validation(iteration, testing_iterations, testing_interval, iteration < model.gaussian.delay, scene, scene.test_dataset_6k, evaluator,
                #            (pipe, background))
                scene.save_checkpoint(iteration)



def validation(iteration, testing_iterations, testing_interval, delay, scene: Scene, testset, evaluator, renderArgs):
    ...
    print("test_samples:",len(testset))
    for config in validation_configs:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                data = testset[data_idx]
                render_pkg = render(data, iteration, scene, *renderArgs, compute_loss=True, return_opacity=True, delay=delay)
                examples = []
                image = torch.clamp(render_pkg["render"], 0.0, 1.0)
                gt_image = torch.clamp(data.original_image.to("cuda"), 0.0, 1.0)
                opacity_image = torch.clamp(render_pkg["opacity_render"], 0.0, 1.0)
                ...
                if idx % 40 == 0:
                    ...
                    wandb_img = wandb.Image(full_image[None],
                                            caption=str(len(testset))  + "_view_{}/full_render".format(data.image_name))
                    examples.append(wandb_img)
                    ...
                    #  cropped
                    render_ROI,_ = testset.generate_patch_image(full_image.permute(1,2,0).detach().cpu().numpy(), data.bbox.detach().cpu().numpy(), [256, 256])
                    ground_truth_ROI,_ = testset.generate_patch_image(full_gt_image.permute(1,2,0).detach().cpu().numpy(), data.bbox.detach().cpu().numpy(), [256, 256])
                    wandb_img = wandb.Image((render_ROI * 255).astype(np.uint8),
                                            caption=str(len(testset)) + "_view_{}/roi_render".format(data.image_name))
                    examples.append(wandb_img)
                ...
                if config['name'] == 'test':
                    ...
                    # filter bg images
                    if len(np.where(data.full_mask.cpu().numpy())[0]) == 0:
                    ...
                    #
                    # ADDS, MCE = compute_obj_metrics_ycb(render_pkg["updated_camera"].obj_rots_gt,
...
def debug(scene: Scene, testset, evaluator, tag, renderArgs):
    debug_dir = '/mnt/sda1/lxy/debug_render/'+ tag
    ...
    for config in validation_configs:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                if idx % 10 == 0:
                    data = testset[data_idx]
                    render_pkg = render(data, 360001, scene, *renderArgs, compute_loss=True, return_opacity=True, delay=False)
                    examples = []
                    image = torch.clamp(render_pkg["render"], 0.0, 1.0)
                    gt_image = torch.clamp(data.original_image.to("cuda"), 0.0, 1.0)
                    opacity_image = torch.clamp(render_pkg["opacity_render"], 0.0, 1.0)
                    ...
                    cv2.imwrite(os.path.join(debug_dir, '{}_full_render_{}.png'.format(idx, str(data.image_name))),full_render)
                    cv2.imwrite(os.path.join(debug_dir, '{}_full_ground_truth_{}.png'.format(idx, str(data.image_name))),
                                full_ground_truth)

                    render_ROI, _ = testset.generate_patch_image(full_image.permute(1, 2, 0).detach().cpu().numpy(),
                                                                 data.bbox.detach().cpu().numpy(), [256, 256])
                    ground_truth_ROI, _ = testset.generate_patch_image(
                        full_gt_image.permute(1, 2, 0).detach().cpu().numpy(), data.bbox.detach().cpu().numpy(),[256, 256])

                    cv2.imwrite(os.path.join(debug_dir, '{}_roi_render_{}.png'.format(idx, str(data.image_name))),
                                cv2.cvtColor((render_ROI * 255).astype(np.uint8), cv2.COLOR_BGR2RGB))
                    cv2.imwrite(os.path.join(debug_dir, '{}_roi_ground_truth_{}.png'.format(idx, str(data.image_name))),
            ...
            psnr_test /= (len(config['cameras'])/sample-render_eval_filter_num)
            ssim_test /= (len(config['cameras'])/sample-render_eval_filter_num)
            lpips_test /= (len(config['cameras'])/sample-render_eval_filter_num)
            l1_test /= (len(config['cameras'])/sample-render_eval_filter_num)
            mpjpe /= (len(config['cameras'])/sample)
            mpjpe_mano /= (len(config['cameras'])/sample)
            e_ADDS /= (len(config['cameras'])/sample)
            e_MCE /= (len(config['cameras'])/sample)
            e_OCE/= (len(config['cameras'])/sample)
            print("\n[ITER {}] Evaluating {}: lpips {} PSNR {} MPJPE {} OCE {}".format(360001, config['name'], lpips_test, psnr_test, mpjpe, e_OCE))
            wandb.log({
                config['name'] + str(len(testset))+' - psnr' : psnr_test,
                config['name'] + str(len(testset))+' - ssim': ssim_test,
                config['name'] + str(len(testset))+' - lpips': lpips_test,
...
#@hydra.main(version_base=None, config_path="configs", config_name="config_3dgs_avatar")
#@hydra.main(version_base=None, config_path="configs", config_name="config_baseline")
@hydra.main(version_base=None, config_path="configs", config_name="config")
def main(config):
    # print(OmegaConf.to_yaml(config))
    OmegaConf.set_struct(config, False)  # allow adding new values to config
    # print(config.name)
    config.exp_dir = config.get('exp_dir') or os.path.join('/mnt/sda1/lxy/HOGS_results/', config.name)
    ...
    if config.save_code:
        shutil.copyfile('train_dexycb.py', config.exp_dir + '/code/train_dexycb.py')
    ...
    training(config, debug_render=True)
