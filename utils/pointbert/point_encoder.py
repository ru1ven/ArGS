...
from utils.pointbert.dvae import Group
from utils.pointbert.dvae import Encoder
from utils.pointbert.logger import print_log

from utils.pointbert.checkpoint import get_missing_parameters_message, get_unexpected_parameters_message
...
class PointTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        if not self.args.evaluate_3d:
            self.load_model_from_ckpt('../lib/pretrained/pointbert_ULIP-2.pt')
...
class PointTransformer_Colored(nn.Module):
    def __init__(self, config, channnel_size):
        super().__init__()
        self.config = config
        ...
        self.encoder.first_conv[0] = nn.Conv1d(channnel_size, 128, 1)
        self.cls_head_finetune = nn.Sequential(
    ...
    def forward(self, pts):
        pts = pts.permute(0, 2, 1)  # B N C
        # divide the point cloud in the same form. This is important
        neighborhood, center, fps = self.group_divider(pts)
        # encoder the input cloud blocks
        group_input_tokens = self.encoder(neighborhood)  # B G N
        group_input_tokens = self.reduce_dim(group_input_tokens)
        # prepare cls
