...
def getProjectionMatrix_offaxis(znear, zfar, intrx, width, height):
    # 计算near面左右上下边界，单位是与相机坐标系一致的距离（米）
    fx, fy, cx, cy = intrx[0, 0], intrx[1, 1], intrx[0, 2], intrx[1, 2]
    left = - (cx) / fx * znear
    right = (width - cx) / fx * znear
    bottom = - (cy) / fy * znear
    top = (height - cy) / fy * znear

    P = torch.zeros(4, 4)

    z_sign = 1.0  # 根据你的原代码，这里保持1.0，OpenGL一般为-1.0，视API而定
...
def axis_angle_to_matrix(axis: torch.Tensor, angle: torch.Tensor) -> torch.Tensor:
    """
    Rodrigues 公式计算旋转矩阵，保证梯度可回传
    axis: (3,) 旋转轴
    angle: float 旋转角（弧度）
    """
    axis = axis / (axis.norm() + 1e-8)  # 归一化旋转轴
    x, y, z = axis
    ...
    R = torch.zeros(3, 3, device=axis.device, dtype=axis.dtype)
