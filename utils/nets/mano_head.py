...
class ManoHead(nn.Module):
    ...
    def forward(self, pose6d, shape, mano_params=None):
        ...
        if C == 6:
            ...
            pose_rotmat = pose6d.permute(0, 2, 1, 3).view(L, B, N, 3, 3)
            pose = rotation_matrix_to_angle_axis(pose_rotmat)
            pose = pose.contiguous().view(-1, self.mano_pose_size)
        elif C == 3:
            pose = pose6d.permute(0, 2, 1, 3).contiguous().view(-1, self.mano_pose_size)
            pose_rotmat = angle_axis_to_rotation_matrix(pose.view(-1, 3)).view(L, B, N, 3, 3)
            #pose = pose.contiguous().view(-1, self.mano_pose_size)
        ...
        pred_mano_results = {
            ...
            "mano_pose6d": pose.view(L, B, pose.shape[-1]),
            "mano_shape": shape,
        }

        if mano_params is not None:
            gt_mano_shape = mano_params[:, self.mano_pose_size :self.mano_pose_size+10]
