...
class JointvoteLoss(nn.Module):
    ...
    def forward(self, hand_points, hand_off, hand_cls, joint_gt, init_joint):
        ...
        # hand_vote = (
        #         init_joint.unsqueeze(1).unsqueeze(0).expand(l, b, p, j, 3)
        #         + hand_off.reshape((l, p, b, j, 3)).permute(0, 2, 1, 3, 4).contiguous()
        # )
        ...
        #hand_joints = torch.mean(hand_vote, dim=2)
