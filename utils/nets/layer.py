...
class MLP(nn.Module):
    ...
    def __init__(
        self, input_dim, hidden_dim, output_dim, num_layers, is_activation_last=False
    ):
        super().__init__()
        ...
        if not isinstance(hidden_dim, list):
            ...
            assert isinstance(
                hidden_dim, list
            ), "hidden_dim arg should be list or a number"
        ...
        self.layers = nn.ModuleList(
            nn.Linear(all_dims[i], all_dims[i + 1]) for i in range(num_layers)
        )

        torch.nn.init.normal_(self.layers[-1].weight, mean=0., std=1e-5)
        torch.nn.init.constant_(self.layers[-1].bias, val=0.)
