...
from utils.nets.layer import (
    make_conv_layers,
    make_deconv_layers,
)
from utils.nets.resnet import ResNetBackbone
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.convOut_hm = make_conv_layers(
            [32, 32, 1], kernel=1, padding=0, bnrelu_final=False
        )
        self.convOut_hand_seg = make_conv_layers(
            [32, 32, 1], kernel=1, padding=0, bnrelu_final=False
        )
        self.convOut_obj_seg = make_conv_layers(
            [32, 32, 1], kernel=1, padding=0, bnrelu_final=False
        )
    ...
    def forward(self, img_feat, skip_conn_layers):
        ...
        deconv_img_feat4_cat_conv = self.conv4(
            deconv_img_feat4_cat
        )  # 16x128x128 (featxhxw)
        feature_pyramid["stride2"] = deconv_img_feat4_cat_conv

        heatmap_out = self.convOut_hm(deconv_img_feat4_cat_conv)  # N x 1 x 128 x 128
        hand_seg_out = self.convOut_hand_seg(
            deconv_img_feat4_cat_conv
        )  # N x 1 x 128 x 128
        hand_seg_out = hand_seg_out.sigmoid()
        obj_seg_out = self.convOut_obj_seg(
            deconv_img_feat4_cat_conv
        )  # N x 1 x 128 x 128
        obj_seg_out = obj_seg_out.sigmoid()
        heatmap_out = torch.cat([heatmap_out, hand_seg_out, obj_seg_out], dim=1)
...
class Decoder_big(nn.Module):
    def __init__(self):
        ...
        self.convOut_hm = make_conv_layers(
            [128, 128, 64, 1], kernel=1, padding=0, bnrelu_final=False
        )
        self.convOut_hand_seg = make_conv_layers(
            [128, 128, 64, 1], kernel=1, padding=0, bnrelu_final=False
        )
