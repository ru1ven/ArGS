...
from models.deformer.rigid import ObjRevoluteDeform
...
from utils.general_utils import fix_random, PSEvaluator, tensor_to_numpy_image, update_frame_weights, save_deltas
...
def training(config):
    ...
    testing_iterations = [2500, 5000, 7500, 10000]

    testing_interval = 2500
    saving_iterations = [2500, 5000, 7500, 10000]
    ...
    scene.load_checkpoint(checkpoint, strict=False)

    for sub_id in gaussians_hand_group:
        gaussians_hand_group[sub_id]['right'].refine_setup(opt)
        gaussians_hand_group[sub_id]['left'].refine_setup(opt)
    for obj_id in gaussians_obj_group:
        gaussians_obj_group[obj_id].refine_setup(opt)

    getattr(scene.converter, f"deformer_obj_{list(scene.gaussians_obj_group.keys())[0]}").rigid =\
        ObjRevoluteDeform(config.model.deformer.rigid, scene.metadata_obj, 'obj').cuda()
    ...
    num_frames = len(scene.train_dataset)
    with torch.no_grad():
        render_pkg = render(scene.train_dataset[0], 1, scene, pipe, background, compute_loss=True, return_opacity=True,
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        with torch.no_grad():
            validation(iteration, testing_iterations, testing_interval, True, scene, evaluator, (pipe, background))
        ...
        if not data_stack:
            #data_stack = [0, 50, 130, 144, 192, 226]
        ...
        novel_cacmera = scene.test_dataset[data_idx]
        # Render
        ...
        render_pkg = render(data, iteration, scene, pipe, background, compute_loss=True, return_opacity=True,
                                white_bg=dataset.white_background, delay=True, novel_data=novel_cacmera)
        ...
        if iteration % 1000 == 0:
            ...
            delta_norm = save_deltas({0: render_pkg['movable_prob']},
...
def validation(iteration, testing_iterations, testing_interval, delay, scene: Scene, evaluator, renderArgs):
    ...
    ssim_train = 0.0
    lpips_train = 0.0
    #test_list = [0, 50, 130, 144, 192, 226]
    test_list = scene.train_dataset
    for idx, data in enumerate(scene.train_dataset):
        #data = scene.train_dataset[idx]
        ...
        if iteration<=1:
            ...
            delay = True
        render_pkg = render(data, iteration+idx, scene, *renderArgs, compute_loss=True, return_opacity=True, delay=delay, novel_data=novel_cacmera)
        examples = []
    ...
    psnr_test /= len(test_list)
    ssim_test /= len(test_list)
    lpips_test /= len(test_list)
    psnr_train /= len(test_list)
    ssim_train /= len(test_list)
    lpips_train /= len(test_list)
...
@hydra.main(version_base=None, config_path="configs", config_name="config_arctic_refine")
def main(config):
    # print(OmegaConf.to_yaml(config))
    OmegaConf.set_struct(config, False)  # allow adding new values to config
    # print(config.name)
    config.exp_dir = config.get('exp_dir') or os.path.join('/mnt/sda2/lxy/NonrigidGS_results/', config.dataset._YCB_CLASSES[0],"REFINE"+config.name)
    ...
    if config.save_code:
        shutil.copyfile('train_bone.py', config.exp_dir + '/code/train_bone.py')
